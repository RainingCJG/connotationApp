<template>
  <div id="video" class="mescroll">
    ...
    <!-- 有数据时父子组件数据传递 -->
    <ul else-if class="content" id="video-datalist">
...
export default {
  ...
  data () {
    return {
      type: '-104', // 消息类型
      items: Store.fetch('video').slice(0, 20), // 从缓存中读取数据
      preTime: 0, // 上次刷新时间
  ...
  mounted () {
    this.mescroll = new MeScroll('video', {
      down: {
  ...
  methods: {
    ...
    // 设置数据
    setResData (data) {
      let preStoreData = Store.fetch('video')
      // this.items = data.concat(this.items)
      for (let i in data) {
        this.items.unshift(data[i])
      }
      Store.save('video', data.concat(preStoreData))
      this.mescroll.endSuccess()
      // 刷新成功提示信息
      this.tip = this.reflashTip
      // 当localstorage容量满时删除一些旧的数据
      let jugeStoreRoom = Store.fetch('video')
      if (jugeStoreRoom.length >= 120) {
        for (let i = 0; i < 20; i++) {
          jugeStoreRoom.pop()
        }
        Store.save('video', jugeStoreRoom)
      }
    },
    // 下拉刷新方法
    downCallback () {
    ...
    upCallback (page) {
      ...
      setTimeout(() => {
        this.items = this.items.concat(Store.fetch('video').slice(len, len + 10))
        this.mescroll.endBySize(this.items.length, Store.fetch('video').length)
      }, 1000)
    }
  }
}

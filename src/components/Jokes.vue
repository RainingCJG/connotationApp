<template>
  <div id="jokes" class="mescroll">
    ...
    <!-- 有数据时父子组件数据传递 -->
    <ul else-if class="content" id="jokes-datalist">
...
export default {
  ...
  data () {
    return {
      type: '-102', // 消息类型
      items: Store.fetch('jokes').slice(0, 20), // 从缓存中读取数据
      preTime: 0, // 上次刷新时间
  ...
  mounted () {
    this.mescroll = new MeScroll('jokes', {
      down: {
  ...
  methods: {
    ...
    // 设置数据
    setResData (data) {
      let preStoreData = Store.fetch('jokes')
      // this.items = data.concat(this.items)
      for (let i in data) {
        this.items.unshift(data[i])
      }
      Store.save('jokes', data.concat(preStoreData))
      this.mescroll.endSuccess()
      // 刷新成功提示信息
      this.tip = this.reflashTip
      // 当localstorage容量满时删除一些旧的数据
      let jugeStoreRoom = Store.fetch('jokes')
      if (jugeStoreRoom.length >= 120) {
        for (let i = 0; i < 20; i++) {
          jugeStoreRoom.pop()
        }
        Store.save('jokes', jugeStoreRoom)
      }
    },
    // 下拉刷新方法
    downCallback () {
    ...
    upCallback (page) {
      ...
      setTimeout(() => {
        this.items = this.items.concat(Store.fetch('jokes').slice(len, len + 10))
        this.mescroll.endBySize(this.items.length, Store.fetch('jokes').length)
      }, 1000)
    }
  }
}

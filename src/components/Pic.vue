<template>
  <div id="pic" class="mescroll">
    ...
    <!-- 有数据时父子组件数据传递 -->
    <ul else-if class="content" id="pic-datalist">
...
export default {
  ...
  data () {
    return {
      type: '-103', // 消息类型
      items: Store.fetch('pic').slice(0, 20), // 从缓存中读取数据
      preTime: 0, // 上次刷新时间
  ...
  mounted () {
    this.mescroll = new MeScroll('pic', {
      down: {
  ...
  methods: {
    ...
    upCallback (page) {
      ...
      setTimeout(() => {
        this.items = this.items.concat(Store.fetch('pic').slice(len, len + 10))
        this.mescroll.endBySize(this.items.length, Store.fetch('pic').length)
      }, 1000)
    }
  }
}
